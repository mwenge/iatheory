...
In order to get something to work with, we must first somehow convert the contents of the
cassette tape into a binary file and then emulate the steps the C64's tape player performed
...
\icode{31} & \icode{31} & \icode{30} & \icode{30} & \icode{31} & \icode{30} & \icode{32} & \icode{31} & \icode{30} & \icode{31} & \icode{30} & \icode{30} & \icode{32} & \icode{31} & \icode{30} & \icode{30}  \\
      ...
      \end{tikzpicture}
    \end{adjustbox}
  }\caption{The first batch of data we read in from the tape. The 'Header Description' is in grey, the Version Number in pink, Reserved bytes in blue, and the File Size in red.}
\end{figure}
...
\begin{lstlisting}[caption=The data we have just loaded\, translated back into assembly language. This is the MegaSave loader.]
...
This sequence\index{sequence} has the catchy name of a 'Sync Train':
...
When it has completed it has loaded each of the four chunks to the following positions in RAM:
