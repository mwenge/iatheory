...
.. referenced in \icode{UpdateEntryScreenRainbow\index{UpdateEntryScreenRainbow}}. After every full screen\index{screen} paint we update the an
...
What we have done is take the two values at index \icode{\$00} in \icode{cellFirstColumnArray\index{cellFirstColumnArray}}
...
The values at index \icode{\$10} in \icode{cellFirstColumnArray\index{cellFirstColumnArray}} are \icode{\$60,\$61} and
in \icode{cellSecondColumnArray\index{cellSecondColumnArray}} are \icode{\$61,\$63}. Translated to row and column position
this gives:
...
two lines its writing by using the \icode{scrollLineOffset\index{scrollLineOffset}} variable to determine which
one its writing.
