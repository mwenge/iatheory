...
% Reserve bytes for the executable we write directly to the finished PDF.
% This is done by the steps in ../notebooks/Add Executable To PDF.ipynb
% We can't add the raw data of the binary here as it would confuse tex.
%\input{src/include_exe.tex}

% Usage of /href
% Disable decoration
\ifx\forprint\undefined
\usepackage{hyperxmp}[2020/03/01]
\usepackage[pdfa,pdfusetitle,hidelinks]{hyperref}
\usepackage{mmap} %only needed for pdfTeX
\hypersetup{%
pdflang=la
pdfapart=3,
pdfaconformance=b,
pdftitle={Iridis Alpha Theory},%
pdfauthor=Rob Hogan,%
pdfsubject={The Making and Mechanics of Iridis Alpha},%
pdfkeywords={C64, 6502, Llamasoft, Iridis Alpha, Jeff Minter}%
}
\else
\usepackage[hidelinks]{hyperref}
\fi

% Set up the marigins for print/pdf as appropriate.
% We want a larger margin for the gutters in the print version.
\ifx\forprint\undefined
\usepackage[margin=1in]{geometry}                                       
\else
\usepackage[inner=1.4in,outer=0.8in]{geometry}                                       
\fi
...
\newtcblisting{mycode}{
  ...
  boxrule=1pt,
  listing only,
  listing options={style=6502Style},
  breakable
}
...
\definecolor{boxyellow}{HTML}{ffffff}
...
\ifstrempty{#1}%
  ...
  \node[font={\small},draw = black, line width = 0.5pt, text = black, anchor=east,rectangle,
