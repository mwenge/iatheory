...
The routine for processing these bytes appears in \icode{UpdateAttackShipDataForNewShip}. 
...
Byte 15 & \$40                       & Update rate for periodically switching to wave defined in Bytes 16-17. \\
 ...
 Byte 17 & planet1Level1Data2ndStage & Hi Ptr to the wave data we switch to periodically.               \\
      ...
      \end{tabular}
    \end{adjustbox}
  }\caption{\icode{planet1Level1Data}: Bytes 15 to 17 for Level 1 on the Sheep Planet.}
\end{figure}

The way it works is Byte 15 is loaded to the appropriate position in \icode{updateRateForAttackShips}:
...
This is then periodically decremented in \icode{GetNewShipDataFromDataStore}. When it reaches zero we 
end up jumping to \icode{SwitchToAlternatingWaveData}:
\begin{lstlisting}
...
In the case of Level 1 this second set of wave data is at \icode{planet1Level1Data2ndStage}. This data is more
or less the same as \icode{planet1Level1Data} with the exception of the bytes in the table below.
...
periodic decrementing of Byte 15 reaches zero again it will switch back to \icode{planet1Level1Data}. This 
...
\begin{figure}[H]
 ...
 Byte 29 & spinningRings             & Hi Ptr for data to switch to when hit by bullet.                                    \\
      ...
      \end{tabular}
    \end{adjustbox}
  }\caption{\icode{planet1Level1Data}: Bytes 28 to 29 for Level 1 on the Sheep Planet.}
\end{figure}
...
\begin{figure}[H]
 ...
 Byte 31 & defaultExplosion          & Hi Ptr for data to switch to after colliding with gilby.                \\
      ...
      \end{tabular}
    \end{adjustbox}
  }\caption{\icode{planet1Level1Data}: Bytes 30 to 31 for Level 1 on the Sheep Planet.}
\end{figure}
...
These bytes are only inspected when responding to a collision. The routine \icode{UpdateEnergyLevelsAfterCollision}
...
use of Bytes 32-23. But there appears to be a flaw here: the pointer is to \icode{\$1488}, a position in memory
...
\midrule
      ...
      \textbf{12} & planet1Level5Data & planet5Level12Data & \icode{\$1488}       \\
