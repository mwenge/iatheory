...
If you look closely you'll notice that \icode{PerformMainGameUpdate\index{PerformMainGameUpdate}}, our so-called boiler-room routine\index{routine}, is called last, when the
...
As you can see in the code comment above the planet\index{planet} surface\index{surface} itself is 1024 bytes long and we store 4 layers of 1024
...
Every time we scroll by one or more characters along the planet\index{planet} surface\index{surface}, updating what we see on the screen\index{screen} will
...
SetUpGilbySprite%\index{SetUpGilbySprite}%
        LDA #GILBY_AIR_RIGHT%\index{GILBY\_AIR\_RIGHT}%%\index{GILBY\_AIR\_RIGHT}%
...
store both the speed \textit{and} direction of the gilby. When the value is between \icode{00} and \icode{80} the gilby is moving
to the left and the value reflects its relative velocity. When the value is between \icode{80} and \icode{FF} the gilby is
...
in temporal order. Notice the way the volume steadily decreases as we go through the \icode{PLAY\_LOOP} loop towards the end.
