...
\begin{figure}[H]
{
  ...
  \includegraphics{music/title_no_1_page_1001.png}%
    \end{adjustbox}
...
\begin{lstlisting}[caption=\icode{MaybePlayVoice1\index{MaybePlayVoice1}}\, part of \icode{PlayTitleScreenMusic\index{PlayTitleScreenMusic}}.,escapechar=\%]
MaybePlayVoice1%\index{MaybePlayVoice1}%   
        DEC voice1NoteDuration%\index{voice1NoteDuration}%
        BNE MaybePlayVoice2%\index{MaybePlayVoice2}%
...
This snppiet gives us the value in \icode{A} written to each register\index{register} for Voice 1. For example, \icode{\$61}
...
\end{definition}
\clearpage
\begin{definition}[Extracting the Title Music cont.]
\setlength{\intextsep}{0pt}%
\setlength{\columnsep}{3pt}%
\small

\lstinputlisting[caption=Title Tune No 1 in ABC format, basicstyle=\tiny\ttfamily]{music/title_no_1_page_1.abc}

  We can then use the tool \icode{abcm2ps} to transform this into an SVG image file giving the music in
