...
The race is now on to get the title sequence\index{sequence} up on the screen\index{screen}. After a little more setup in \icode{MainControlLoop\index{MainControlLoop}}
we call a routine\index{routine} to set up the main title screen\index{screen}:
...
You'll notice we've changed our interrupt\index{interrupt} handler again, this time to a routine\index{routine} called
\icode{TitleScreenInterruptHandler\index{TitleScreenInterruptHandler}}. What we also do is make this interrupt\index{interrupt} something
...
screen\index{screen}? The simple answer is that thanks to Raster Interrupts we are not: when we run
...
\begin{lstlisting}[caption= The 'stripe' character\index{character}.,basicstyle=\tiny,escapechar=\%]
...
will refer to later when it is actually writing dots to the screen\index{screen}. If we write a stripe character\index{character} to a particulas position
...
The painting of sprites\index{sprites} and playing of music as the screen\index{screen} gets painted\index{painted} is all handled by \icode{TitleScreenAnimation\index{TitleScreenAnimation}}.
...
What our diagram above also tells us is that in this visit from the beam \icode{TitleScreenAnimation\index{TitleScreenAnimation}} chose to execute
...
raster doesn't wait for us. We have to work quickly, especially if we're preparing graphics\index{graphics} on lines that its likely
...
we're going to manage this value for the starfield\index{starfield} is by keeping array \icode{titleScreenStarfieldMSBXPosArray\index{titleScreenStarfieldMSBXPosArray}}
...
\icode{spriteMSBXPosOffset\index{spriteMSBXPosOffset}} but if we have \icode{\$80} in there it would set it to 1: 
...
in the routine\index{routine} \icode{Recalculate\-JumpingGilbyPositions} when we are nearer the bottom of the screen\index{screen}. This means the
...
as with the starfield\index{starfield} we need a separate array (\icode{titleScreen\-GilbiesMSBXPosArray} to handle this in addition
...
and as the raster progress our gilbies\index{gilbies} are revealed.
