...
And that the routine\index{routine} we'll run when that happens is \icode{TitleScreenInterruptHandler\index{TitleScreenInterruptHandler}}:
...
The painting of sprites\index{sprites} and playing of music as the
screen\index{screen} gets painted\index{painted} is all handled by
\icode{TitleScreen\-Animation\index{TitleScreenAnimation}} the one and only
...
\end{lstlisting}
\icode{TitleScreenAnimation} works by calling one of three different sub-routines. It picks the one to run
